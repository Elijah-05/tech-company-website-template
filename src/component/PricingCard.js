    ...
    <div
      className={`group pricing-container bg-slate-200  sm:w-[320px] max-w-[370px] min-h-[470px] rounded-xl flex flex-col justify-between py-6 px-4 lg:px-4 md:px-3  ${
        name.toLowerCase() === "pro"
          ? "scale-105 hover:scale-[1.06]"
          : "lg:hover:scale-[1.005]"
      }  hover:-translate-y-2 hover:shadow-xl duration-300 my-4 lg:my-0 mx-1 `}
    >
      <div className=" mt-3">
      ...
      <button
        className={`${
          name.toLowerCase() === "pro"
            ? "bg-secondaryColor group-hover:bg-primaryColor"
            : "bg-primaryColor"
        } font-semibold text-white group-hover:text-secondaryColor py-3 rounded-full w-3/4 max-w-40 mx-auto duration-500 `}
      >
        Start Now!
      </button>
    </div>
